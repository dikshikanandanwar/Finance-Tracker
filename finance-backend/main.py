...
def normalize_expense_category(value: Optional[str]) -> str:
    raw = str(value or "").strip().lower()

    if raw in {"housing", "home", "rent", "house"}:
        return "Housing"
    if raw in {"groceries", "grocery", "food"}:
        return "Groceries"
    if raw in {"utilities", "utility", "bill"}:
        return "Utilities"
    if raw in {"shopping", "purchase", "buy"}:
        return "Shopping"
    if raw in {"entertainment", "fun", "movie"}:
        return "Entertainment"

    if any(word in raw for word in ["rent", "flat", "room", "pg", "hostel", "maintenance", "emi", "housing"]):
        return "Housing"
    if any(word in raw for word in ["grocery", "groceries", "vegetable", "milk", "food", "supermarket", "mart", "swiggy", "zomato"]):
        return "Groceries"
    if any(word in raw for word in ["electricity", "water", "internet", "wifi", "gas", "recharge", "utility", "utilities", "bill"]):
        return "Utilities"
    if any(word in raw for word in ["shopping", "amazon", "flipkart", "clothes", "shirt", "shoe", "purchase"]):
        return "Shopping"
    if any(word in raw for word in ["movie", "netflix", "party", "outing", "game", "gaming", "concert", "entertainment"]):
        return "Entertainment"

    return "Other"


def fallback_parse_expense_text(text: str, default_date: Optional[str] = None) -> dict:
    compact = text.replace(",", "")
    amount_match = re.search(r"(\d+(?:\.\d{1,2})?)", compact)
...
@app.post("/parse_expense_text")
async def parse_expense_text(req: ExpenseParseRequest):
    text = (req.text or "").strip()

    if not text:
        raise HTTPException(400, "Expense text is required")

    if client is None:
        fallback = fallback_parse_expense_text(text, req.default_date)
        if fallback["amount"] <= 0:
            raise HTTPException(400, "Could not identify a valid amount from the text")
        return {
            "category": fallback["category"],
            "amount": fallback["amount"],
            "note": fallback["note"],
            "expense_date": fallback["expense_date"],
            "confidence": fallback["confidence"],
            "original_text": text,
        }

    prompt = f"""
    You are an expense parser for a personal finance web app.

    Allowed categories only:
    Housing, Groceries, Utilities, Shopping, Entertainment, Other

    Read the user's sentence and extract exactly one expense entry.

    Return ONLY strict JSON in this format:
    {{
      "category": "Housing|Groceries|Utilities|Shopping|Entertainment|Other",
      "amount": 0,
      "note": "short useful note",
      "expense_date": "{req.default_date or ''}",
      "confidence": "high|medium|low"
    }}

    Rules:
    - amount must be numeric only
    - no currency symbol in amount
    - keep note short and useful
    - if no date is clearly mentioned, use default_date
    ...
    try:
        ...
        category = normalize_expense_category(parsed.get("category"))
        note = str(parsed.get("note") or text).strip()
        expense_date = parsed.get("expense_date") or req.default_date
