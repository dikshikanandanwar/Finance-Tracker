...
function Prediction({ theme }) {
  ...
  const buildExpenseRows = (userId) => {
    ...
    // Merge standard 'Other' with custom expenses under the 'Other' category note
  ...
  // Direct Save Function - Bypass AI Model
  const saveExpenseItems = async (e) => {
    ...
    try {
      ...
      if (!error) {
        toast.success("Expenses saved to Database ✅");
      } else {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '16px' : '20px' }}>
          ...
          <div className="card">
            <form onSubmit={handleAnalyze}>
              ...
              {/* Added Header with Date Picker inline */}
