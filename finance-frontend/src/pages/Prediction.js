...
function Prediction({ theme }) {
  ...
  const saveExpenseItems = async (e) => {
    ...
    try {
      ...
      // 2. Calculate Dashboard Metrics locally (Bypassing AI)
      const savingsPotential = monthlyIncome - totalExpense;
      let health = 'Average';
      ...
      let risk = 'High';
      if (monthlyIncome > 0) {
        const expenseRatio = (totalExpense / monthlyIncome) * 100;
        if (expenseRatio >= 90) risk = 'High';
        else if (expenseRatio >= 70) risk = 'Medium';
        else risk = 'Low';
      }

      // 3. Save the overall summary to 'financial_progress' so it appears on the Dashboard
      const progressPayload = {
        ...
        // Optional: Ensure the creation date matches the picked date
