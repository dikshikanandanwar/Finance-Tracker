...
function TestCorner() {
  ...
  const generateTest = async (subject) => {
    setLoading(true);
    setQuestions([]); setAnswers({}); setScore(null); setIsSubmitted(false);
    ...
    } catch (err) { toast.error("Failed to load module."); }
    ...
    <div className="page-container">
      ...
      {!questions.length && !loading && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
          {subjects.map(sub => (
            <div key={sub.id} className="card" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '15px' }} onClick={() => generateTest(sub)}>
      ...
      {questions.length > 0 && (
        <div className="card">
          {isSubmitted && (
            <div style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid var(--primary)', padding: '20px', borderRadius: '15px', marginBottom: '20px', textAlign: 'center' }}>
          ...
          {questions.map((q, idx) => (
            <div key={idx} style={{ marginBottom: '30px', paddingBottom: '20px', borderBottom: '1px solid var(--glass-border)' }}>
              <h3 style={{ margin: '0 0 15px', fontSize: '1.1rem', color: 'var(--text-dark)' }}>{idx + 1}. {q.question}</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {q.options.map(opt => {
                  let bg = 'var(--input-bg)'; let border = '2px solid transparent';
                  if (answers[idx] === opt) { bg = 'rgba(59, 130, 246, 0.1)'; border = '2px solid #3B82F6'; }
                  if (isSubmitted) {
                    if (opt === q.correct_answer) { bg = 'rgba(16, 185, 129, 0.1)'; border = '2px solid var(--primary)'; }
                    else if (answers[idx] === opt) { bg = 'rgba(239, 68, 68, 0.1)'; border = '2px solid #EF4444'; }
                  }
                  return (
                    <button key={opt} disabled={isSubmitted} onClick={() => setAnswers({...answers, [idx]: opt})}
                      style={{ padding: '15px', borderRadius: '12px', border, background: bg, color: 'var(--text-dark)', textAlign: 'left', fontWeight: 600, cursor: isSubmitted ? 'default' : 'pointer' }}>
                      {opt}
                    </button>
                  );
                })}
          ...
          {!isSubmitted ? (
            <button className="btn-primary" onClick={submitTest}><CheckCircle size={18} style={{marginRight: 8, verticalAlign: 'middle'}}/> Submit Module</button>
          ) : (
            <button className="btn-primary" style={{ background: 'var(--input-bg)', color: 'var(--text-dark)', border: '1px solid var(--input-border)' }} onClick={() => setQuestions([])}><RefreshCw size={18} style={{marginRight: 8, verticalAlign: 'middle'}}/> Return to Menu</button>
          )}
        </div>
      )}
    </div>
