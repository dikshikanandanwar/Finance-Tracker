...
const ExpenseTrendChart = ({ history, theme }) => {
  ...
  const chartOptions = {
    ...
    scales: {
      x: { 
          grid: { display: false }, 
          ticks: { font: { family: "'Inter', sans-serif", weight: '500' }, color: isDark ? '#64748B' : '#94A3B8' } 
      },
      y: { 
  ...
  if (!history || history.length === 0) {
    return (
        <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-light)', fontWeight: 600 }}>
