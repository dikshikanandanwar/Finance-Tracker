...
function Visualize({ theme }) {
  ...
  const generateInsights = (data) => {
    ...
    if (latest.savings_potential < latest.desired_savings)
      tips.push(`Action Required: Missing savings goal by $${(latest.desired_savings - latest.savings_potential).toFixed(2)}.`);
  ...
  const stockChartOptions = {
    ...
    plugins: {
      legend: {
        position: 'top',
        labels: { color: isDark ? '#94A3B8' : '#64748B', font: { family: "'Inter', sans-serif", weight: '600' } }
      },
      tooltip: {
            ...
            {pieChartData && (
              <Doughnut
                ...
                options={{
                  maintainAspectRatio: false,
                  plugins: {
                    legend: { position: 'bottom', labels: { color: isDark ? '#94A3B8' : '#64748B', padding: 16 } }
                  }
                }}
              />
            )}
