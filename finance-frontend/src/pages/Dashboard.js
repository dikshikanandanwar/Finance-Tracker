...
function Dashboard({ theme }) {
  ...
  useEffect(() => {
    const init = async () => {
      setLoading(true);
      await Promise.all([fetchHistory(), fetchRecentExpenses()]);
      setLoading(false);
    };
    init();
  }, []);
  ...
  const submitQuickAdd = async (e) => {
    ...
    const amountNum = parseFloat(quickAdd.amount);

    if (!quickAdd.amount || Number.isNaN(amountNum) || amountNum <= 0) {
    ...
    try {
      ...
      const selectedCategory = quickAdd.category;
      ...
      const previousAmount =
        previousEntries && previousEntries.length > 0
          ? parseFloat(previousEntries[0].amount || 0)
          : null;
      ...
      if (previousAmount !== null && amountNum > previousAmount) {
        toast.error(
          `Warning: ${selectedCategory} expense is higher than previous by ₹${(
            amountNum - previousAmount
          ).toFixed(2)}`
        );
      } else if (previousAmount !== null && amountNum < previousAmount) {
        toast.success(
          `${selectedCategory} expense is lower than previous by ₹${(
            previousAmount - amountNum
          ).toFixed(2)}`
        );
      } else if (previousAmount !== null && amountNum === previousAmount) {
        toast.success(`${selectedCategory} expense is same as previous entry.`);
      } else {
        toast.success(`${selectedCategory} expense saved successfully!`);
      }
  ...
  const openEditModal = (entry) => {
    ...
    setEditForm({
      ...
      income: entry.income ?? '',
      custom_expenses: parsedOtherMeta.customExpenses.length
        ? parsedOtherMeta.customExpenses
        : [createCustomExpense()]
    });
  };
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      'download',
      `financial_history_${new Date().toISOString().split('T')[0]}.csv`
    );
  ...
  return (
    <div className="page-container">
      ...
      >
        <div>
          <h1 style={{ margin: '0 0 5px', color: 'var(--text-dark)' }}>
            Overview
          </h1>
            ...
            {recentExpenses.map((item, index) => (
              <div
                ...
                style={{
                  ...
                  border: '1px solid var(--input-border)',
                  display: 'grid',
                  gridTemplateColumns: isMobile ? '1fr' : '1.2fr 1fr 1fr 2fr',
                  gap: '10px',
                  alignItems: 'center'
                }}
              >
          ...
          >
            ...
            <form onSubmit={handleUpdateEntry}>
              ...
              >
                ...
                >
                  <div>
                    <div style={{ color: 'var(--text-dark)', fontWeight: 800 }}>
                      Custom Expenses
                    </div>
                    <div
                      style={{
                  ...
                  {(editForm.custom_expenses || []).map((item, index) => (
                    <div
                      ...
                      style={{
                        display: 'grid',
                        gridTemplateColumns:
                          'minmax(0, 1.4fr) minmax(120px, 0.8fr) auto',
                        gap: '10px',
                        alignItems: 'end'
                      }}
                    >
                      <div>
                        ...
                        <input
                          ...
                          value={item.name}
                          onChange={(e) =>
                            handleCustomExpenseChange(item.id, 'name', e.target.value)
                          }
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={item.amount}
                          onChange={(e) =>
                            handleCustomExpenseChange(item.id, 'amount', e.target.value)
                          }
                          placeholder="0"
                          style={inputStyle}
                        />
